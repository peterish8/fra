...
function ScoreCard({ label, score, coverage, version, explanation }: { label: string; score: string; coverage: string; version: string; explanation: string }) {
  return <article className="report-score-card"><h3>{label}</h3><p className="report-score-value">{score}</p><p className="report-score-coverage">Coverage: {coverage}</p><small>{version} · {explanation}</small></article>;
}

export function ReportReader({
  claims = sampleClaims,
  title = "Evidence-led company report",
}: {
  claims?: ClaimRow[];
  title?: string;
}) {
  const [selected, setSelected] = useState<ClaimRow | null>(null);
  return <main className="report-page"><header className="report-page-header"><p>Report reader</p><h1>{title}</h1><p>Separate what the company said from what independent evidence supports. Scores explain quality; they do not decide truth.</p></header><section className="report-quality-strip" aria-label="Report quality"><ScoreCard label="Research Confidence" score="78/100" coverage="82%" version="research-confidence-v1" explanation="Weighted claim quality and evidence coverage." /><ScoreCard label="Evidence Coverage" score="82/100" coverage="82%" version="evidence-coverage-v1" explanation="Materiality-weighted assessed claims." /><ScoreCard label="Disclosure Reliability" score="Not enough data" coverage="34%" version="disclosure-reliability-v1" explanation="Independent sample gate is not met." /><ScoreCard label="Financial/Business Score" score="64/100" coverage="68%" version="financial-business-v1" explanation="Cohort-normalized components." /></section><nav className="report-section-nav" aria-label="Report sections"><a href="#overview">Overview</a><a href="#financials">Financials</a><a href="#claims">Claims</a><a href="#sources">Sources</a><a href="#limitations">Limitations</a></nav><section className="report-summary" id="overview"><h2>Overview</h2><p>Identity and report scope are resolved. Unverified and stale statements remain visible rather than being silently promoted.</p></section><div id="claims"><ClaimsTable claims={claims} onInspect={setSelected} /></div><EvidenceInspector claim={selected} onClose={() => setSelected(null)} /><section className="report-summary" id="limitations"><h2>Limitations</h2><p>Some sources may be unavailable or stale. Missing data is shown as unknown, never as zero.</p></section></main>;
}
