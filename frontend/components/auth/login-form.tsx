...
export function LoginForm() {
  ...
  async function handleSendCode(event: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await sendLoginCode(supabase, email);
    if (!result.ok) {
      ...
      >
        ...
        {step === "email" ? (
          <>
            ...
            <small>We&apos;ll only email a sign-in code to an existing account.</small>
          ...
          <>
            ...
            <button type="submit" className="auth-button" disabled={isBusy}>
              {status.state === "verifying" ? "Verifying…" : "Verify & sign in"}
              <span aria-hidden="true">→</span>
            </button>
